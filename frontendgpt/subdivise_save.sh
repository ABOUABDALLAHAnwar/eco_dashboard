...
export function openFormPopup(title, fields, onSubmit, initialValues = {}) {
  ...
  if (!newWindow) { alert("Autorise les pop-ups pour ce site !"); return; }
...
export default function Dashboard() {
  ...
  const openActionPopup = () => openFormPopup(
    "Ajouter Action",
    [{name:"name",placeholder:"Nom action"},{name:"distance",placeholder:"Distance",type:"number"},{name:"type",placeholder:"Type"}],
    async (values,popup)=>{
      try {
        const res = await fetch("http://localhost:8001/add_user_actions", {
          method:"POST", credentials:'include',
          headers:{"Content-Type":"application/json"},
          body:JSON.stringify({name:values.name, info:{distance:Number(values.distance), type:values.type}})
        });
        if(!res.ok) throw new Error("Erreur ajout action");
        popup.close(); fetchActions();
