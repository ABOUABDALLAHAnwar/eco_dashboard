...
def mock_client_collection(monkeypatch, client_accounts_collection):
    ...
    monkeypatch.setattr(
        variables, "client_accounts_collection", client_accounts_collection
    )

    # Remplace la méthode add_new_user pour ne rien écrire réellement
    monkeypatch.setattr(
        collections_handeler.ClientCollection, "add_new_user", MagicMock()
    )
...
def test_signup_existing_email(client_accounts_collection):
    ...
    response = client.post(
        "/signup", data={"email": "existing@test.com", "password": "123456"}
    )
...
def test_login_success(monkeypatch):
    ...
    # Test login
    response = client.post(
        "/login", data={"username": "test@test.com", "password": "123456"}
    )
